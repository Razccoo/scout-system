...
def player_comparison_radar(df, players, params, low, high, lower_is_better=None):
    ...
    for player in players:
        player_values.append(df[df['Player'] == player][params].iloc[0].tolist())
        player_teams.append(df[df['Player'] == player]['Team within selected timeframe'].iloc[0])
    ...
    for i, (player, season, team) in enumerate(zip(players, player_seasons.values(), player_teams)):
        ...
        title_text = axs['title'].text(title_coords[i % 2][0], title_coords[i % 2][1] - 1 * (i // 2), 
                                    f'{player} | {season}', fontsize=25,
                                    fontproperties=robotto_bold.prop, ha=ha,
                                    va='center', color=color)
        team_text = axs['title'].text(team_coords[i % 2][0], team_coords[i % 2][1] - 1 * (i // 2), 
                                    f'{team}', fontsize=20,
    ...
    ab = AnnotationBbox(imagebox, (0.33, 0.67), frameon=False)
